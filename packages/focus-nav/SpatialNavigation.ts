// TODO: For Yusuf - the type setup for this package is not working right
...
export type PressedKeys = Record<string, number>;
...
export type BackwardsCompatibleKeyMap = Record<
  string,
  string | number | (number | string)[]
>;

export type KeyMap = Record<string, (string | number)[]>;
...
class SpatialNavigationService {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private readonly setFocusDebounced: DebouncedFunc<any>;
  ...
  onEnterPress(keysDetails: KeyPressDetails) {
    ...
    if (component.simulateClick) {
      this.log('onEnterPress', 'simulating click');
      component.node.click();
    }
  }
  ...
  getNextFocusKey(targetFocusKey: string): string {
    ...
    if (children.length > 0) {
      const {
        destination: destinationFocusKey,
        lastFocusedChildKey,
      ...
      this.log('getNextFocusKey', 'destination is', destinationFocusKey);
      /**
       *
       * First of all, forward focus to the destination if possible
       */
      if (
        destinationFocusKey &&
        this.isParticipatingFocusableComponent(destinationFocusKey)
      ) {
        this.log(
          'getNextFocusKey',
          'destination will be focused',
          destinationFocusKey
        );

        return this.getNextFocusKey(destinationFocusKey);
      }

      /**
       * Trying to focus last focused child
       */
      if (
        lastFocusedChildKey &&
    ...
    /**
     * If we reached to this point it means we couldn't focus anything.
     * We might be trying to focus an empty focus container for example.
     * We should try to recover the focus here.
     */
    if (targetComponent.focusContainer) {
      this.log(
        'getNextFocusKey',
        "could't focus, attempting to recovering to the parent",
        targetComponent.parentFocusKey
      );

      /**
       * We tried and couldn't focus to anything for this target already.
       * Keeping this as `focusable` will make it a potential candidate for
       * the next try we're about to make. It can (and will) lead to an infinite loop.
       * So, briefly disabling it to prevent that.
       */
      targetComponent.focusable = false;
      const nextFocusKey = this.getNextFocusKey(targetComponent.parentFocusKey);
      targetComponent.focusable = true;
  ...
  setCurrentFocusedKey(newFocusKey: string, focusDetails: FocusDetails) {
    ...
    if (this.isFocusableComponent(this.focusKey)) {
      ...
      if (this.shouldFocusDOMNode && newComponent.node) {
        newComponent.node.focus({ preventScroll: true });
      }
  ...
  updateFocusable(
    ...
    if (component) {
      ...
      component.focusContainer = focusContainer;
      component.simulateClick = simulateClick;
  ...
  setDestination(focusKey: string, destination?: string) {
    const component = this.focusableComponents[focusKey];
    if (component) {
